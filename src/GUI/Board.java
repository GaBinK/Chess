				...
				if (select == 1) { // 말을 선택했던 상황에서
					if (panel[y][x].getBackground() == Color.pink) { // 이동가능범위 클릭한 경우
						...
						if (state[y][x] / 10 == 1) {
							...
							attack = king.pAttackfrom(Bking_y, Bking_x, state);
							check = check_status.check_1(Bking_y, Bking_x, enemyMoves);
							checkmate = check_status.checkmate(Bking_y, Bking_x, state,
									king.pMoves(state, Bking_y, Bking_x, 3, enemyMoves), enemyMoves, check, 3);
							...
							checkmate = check_status.checkmate(Wking_y, Wking_x, state,
									king.pMoves(state, Wking_y, Wking_x, 1, enemyMoves), enemyMoves, check, 1);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (select == 1) { // 말을 선택했던 상황에서
					if (panel[y][x].getBackground() == Color.pink) { // 이동가능범위 클릭한 경우
						...
						if ((state[y][x] / 10) % 2 == 0) {
							check = check_status.check_1(Bking_y, Bking_x, canAttack)
									|| check_status.check_1(Wking_y, Wking_x, canAttack);
							if (check == true) {
								checkmateB = check_status.checkmate2(Bking_y, Bking_x, state,
										king.pMoves2(state, Bking_y, Bking_y, 3, enemyMoves),
										State.getAllMoves2(state, 1), check, 3);
								checkmateW = check_status.checkmate2(Wking_y, Wking_x, state,
										king.pMoves2(state, Wking_y, Wking_y, 1, enemyMoves),
										State.getAllMoves2(state, 1), check, 1);
							}
						} else {
							check = check_status.check_1(Rking_y, Rking_x, canAttack)
									|| check_status.check_1(Gking_y, Gking_x, canAttack);
							if (check == true) {
								checkmateR = check_status.checkmate2(Rking_y, Rking_x, state,
										king.pMoves2(state, Rking_y, Rking_y, 2, enemyMoves),
										State.getAllMoves2(state, 2), check, 2);
								checkmateG = check_status.checkmate2(Gking_y, Gking_x, state,
										king.pMoves2(state, Gking_y, Gking_y, 4, enemyMoves),
